...
of upstream's 0.3.4/0.3.5 work (OS Media Session, hold-to-2x, number-key
seeking, iOS pseudo-fullscreen, small-player control-bar overhaul, captions
rotate-with-video, 200% volume boost, QoE analytics, framework wrappers,
cold-start audio prime for heavy software codecs, out-of-window range fetch,
HTTP streaming without cross-origin isolation, …) while **keeping this fork's
off-thread worker audio path** (DTS/TrueHD/FLAC decoded in a Web Worker so they
can't peg the main thread) and its HTTP-source resilience work. Best-of-both
where the two overlapped:

- **Audio (both, coexisting):** the worker decode path is preserved AND
  upstream's cold-start audio prime runs on top of it — `usesSoftware` is true
  for the worker, so heavy-codec (TrueHD/MLP/DTS/DCA) priming still engages.
  `flush()`/`reset()` now clear both the failure circuit-breaker (upstream) and
  flush the WASM decoder under our `trackIndex` guard (ours).
- **HTTP source (best-of-both):** upstream's refactor (out-of-window reads via a
  one-off range fetch instead of a stream restart; streaming without
  cross-origin isolation) is the base, and our **`.9` background read-stall
  watchdog** (`READ_STALL_TIMEOUT_MS`) rides on top — upstream's download loop
  still had the un-timed `reader.read()` that watchdog fixes.
- **WASM strings:** kept our single `decodeUtf8()` helper (copies out of the
  resizable heap once) over upstream's per-site inline `.slice()` — same fix,
  DRY-er. The WASM build now exports both `_movi_set_stream_discard` (upstream,
  for `applyStreamDiscard`) and the `_movi_audio_decoder_*` worker API (ours).

### Fixed (merge reconciliation)

- **Premature resume over a black screen on slow forced-timeout seeks** —
  introduced by the merge, caught in post-merge review. Our fork's
  forced-timeout path deliberately stays *sync-armed* and buffers until a real
  video frame lands, and its `canResume` gate carried a `!waitingForVideoSync`
  guard so it wouldn't resume mid-seek. Upstream 0.3.5's `canResume` rewrite
  added prime-aware `dwellMs`-based fallbacks but dropped that guard (safe
  upstream, which *completes* the seek before buffering). Merging our stay-armed
  path with upstream's guardless gate let the `(audioReady && dwellMs >= 3000)` /
  `maxDwell` fallbacks fire ~3 s into a slow seek — resuming over a black screen,
  then snapping back to paused when the frame finally decoded (with audio-packet
  loss). Restored the `!waitingForVideoSync` guard on top of upstream's fallbacks
  (best-of-both), removed the now-dead upstream forced-without-frame branch that
  our earlier early-return shadowed, and added a process-loop regression test
  (`test/core/seek-sync.test.ts`) that fails without the guard.
- **`SoftwareAudioDecoder.flush()`/`reset()` de-duplicated**: the ours+upstream
  union left the two methods with near-identical bodies; both now delegate to a
  single private `clearDecoderState()` so the WASM-flush + circuit-breaker-clear
  + coalesce-queue-clear sequence can't drift between them.
